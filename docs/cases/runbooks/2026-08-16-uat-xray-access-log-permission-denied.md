...
2. 主机上的 `nobody` 和 `caddy` 都已属于 `xray-access` 组；该组为 `agent-svc-plus` role 创建，并由该 role 将两个服务用户加入组。
3. `agent-svc-plus` role 原本将共享日志设置为 `root:xray-access`、`0660`，因此服务用户通过组权限可写。
...
`roles/vhosts/xray-exporter/tasks/main.yml` 对 agent-svc-plus 管理的共享日志错误地设置了 `0644`，覆盖了 `agent-svc-plus` role 建立的 `0660` 组写权限。问题不是 `xray-access` 组未引入：该组及成员关系已经正确；根因是 exporter role 破坏了共享文件的权限契约。
...
`xray-access` 的创建和 `nobody`/`caddy` 成员关系继续由 `agent-svc-plus` role 负责，避免两个 role 各自管理同一权限契约的不同部分。
...
- 修改 exporter 日志权限时，同时检查 agent-svc-plus 的服务用户、组和日志初始化任务。
